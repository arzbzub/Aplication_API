...
root = '/home/arzbzub/Aplication_API/modelo/'
root_db = '/home/arzbzub/Aplication_API/databases/'
...
def get_predict():
    ...
    conn.commit()
    conn.close()
    return "con valores %.1f %.1f %.1f: %s" %(tv, radio, newspaper,str(pred)), 200

@app.route('/review_predict', methods=['GET'])
def return_predicts():
    conn = sqlite3.connect(root_db +"advertising.db")
    crs = conn.cursor()
